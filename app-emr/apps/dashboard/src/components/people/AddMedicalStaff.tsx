...
async function createMedicalStaff({ data }: any) {
  console.log({ data });
  const body = await ky
    .post(apis.medicalStaffs, {
      json: data,
      headers: {
...
export default function AddMedicalStaff() {
  const form = useForm({
    defaultValues: {
      name: "",
      email: "",
  ...
  const { mutate: mutateCreateMedicalStaff } = useMutation({
    mutationKey: ["createMedicalStaff"],
    mutationFn: createMedicalStaff,
  });
